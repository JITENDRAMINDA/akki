...
app = Client ("ssss",bot_token="735294041:AAHxoEk4qSd80m0s34Rk3MgceLdXsXMdsAo",api_id=814511,api_hash="44462f0f278503255d5cc30941b617a9")                                   
  ...
  if id in x:
   try:
     if message.text == ".":   
      ...
      client.edit_message_text(k,int(x[x.index(id)+1]), "**" + message.text.replace("🇩🇪","🇭🇳").replace("🎾","🥎").replace("🖲","🚨").replace("📟","⚱️").replace("WD","🕵️ WIDE BALL 🕵️").replace("/","►").replace("CHALU RAKHO","🤾 GAME STARTED 🚶, ⛹️ PLAYERS ON THE STEDIUM 🏌️").replace("NB","🙅 NO BALL 🙅") + "**" )

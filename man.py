...
def forward(client, message):
 ...
 for word in words:
  if word.casefold() in message.text.casefold():
   f = True
 if not f:
  mes = client.send_message(d,message.text.replace("TRINBAGO","𝕋ℝ𝕀ℕ𝔹𝔸𝔾𝕆").replace("🕵🏻 ","🧚‍♂️ ").replace("🕵🏻","🧚‍♂⛳️").replace("🏳️‍🌈","🇮🇩").replace("☎️","🥁"))
...
def forward(client, message):
 ...
 for line in lines:
  ...
  if id in x:
   try:
    client.edit_message_text(d,int(x[x.index(id)+1]),message.text.replace("TRINBAGO","𝕋ℝ𝕀ℕ𝔹𝔸𝔾𝕆").replace("🕵🏻 ","🧚‍♂️ ").replace("🕵🏻","🧚‍♂⛳️").replace("🏳️‍🌈","🇮🇩").replace("☎️","🥁"))
